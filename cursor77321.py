...
Don't add headings like "Update:â€‌.
...
When implementing any feature, do not stop at the user interface.
...
A feature is NOT complete if only the page or form exists.

Always trace the complete execution path from:
...
- Verify every form submits correctly.
- Verify data is saved.
- Verify data can be edited.
- Verify data can be retrieved.
- Verify data can be deleted if required.
- Verify integrations are connected.

Cross-File Verification

When editing a file, always inspect related files.

Never assume a change only requires one file.
...
Do not mark a task complete until all dependencies are connected.

Kognitix AI Specific Rules

Always assume the platform is a production SaaS system.
...
Any AI feature must actually communicate with the configured AI provider.

Any WhatsApp feature must actually connect to the configured WhatsApp integration.

Any Social Media feature must actually connect to supported social platforms.

Avoid creating placeholder pages.

Avoid creating mock features.

Avoid creating UI-only implementations.

Every feature should be functional unless the user explicitly requests a prototype.

Mobile Dashboard Rules

Dashboard usability has very high priority.
...
Any dashboard that is difficult to use on a phone should be redesigned before considering the task complete.